...
class MovieTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Configure the cell...
        cell.labelChanelMovie.text = "\(chanelName[indexPath.row])"
        cell.labelNumMovie.text = "\(indexPath.row+1)"
        cell.labelSubMovie.text = "\(sub[indexPath.row])"
        cell.labelViewsMovie.text = "\(views[indexPath.row])"
        if cell.labelNumMovie.text == "\(1)" {
            let firstColor = #colorLiteral(red: 0.8441851735, green: 0.6620926857, blue: 0.3147920668, alpha: 1)
            cell.labelNumMovie.textColor = firstColor
        }
        if cell.labelNumMovie.text == "\(2)" {
            let second = #colorLiteral(red: 0.3163542449, green: 0.3409048319, blue: 0.4538033605, alpha: 1)
            cell.labelNumMovie.textColor = second
        }
        if cell.labelNumMovie.text == "\(3)" {
            let third = #colorLiteral(red: 0.3884379268, green: 0.3042663336, blue: 0.1507078409, alpha: 1)
            cell.labelNumMovie.textColor = third
        }
        ...
        DispatchQueue.global().async { [self] in
            ...
            DispatchQueue.main.async {
                ...
                cell.imgMovie.layer.cornerRadius = cell.imgMovie.frame.height/2
            }
        }
